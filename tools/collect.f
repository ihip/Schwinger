...
C     **                   **                                         **
C     ******************************************************************
C     Collects statistical data for different kappa-s:
...
C     - writes on the screen kappa and the name of the computable for
...
C       >>> other variables which are needed
	...
	write(*, *)
	write(*, *) 'Data collector v3 (ivh, 02 Jul 97)'
	write(*, *) '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
	write(*, *)
...
10      read(3, '(a)', end = 99) data_file_name
	...
	write(*, '(1x, ''beta = '', f6.2, ''   kappa = '', f8.4)')
     &    beta, akap
        write(*, *) '  ', c_computable(kcomp)
	write(2, '(''#'', ''beta = '', f6.2, ''   kappa = '', f8.4)')
     &    beta, akap
...
C       >>> nexcp - number of exceptional configurations
	...
	write(2, '(1x, f10.7, e16.8, e11.3, i8, i6)')
     & akap, mean_x, sd_naiv, nmeas, nexcp
...
c	>>> number of jk blocks: from 10 to 25 and then averaged!
	...
	if(akap .eq. 0.0) then
     ...
     & dsqrt(var_sigma2s_all), k, nexcp
	  ...
	  write(2, '(1x, f10.7, e16.8, e11.3, e12.4, e10.2, i8, i4)')
     & akap, aa, dsqrt(var_all), dsqrt(sigma2s_jk),
     & dsqrt(var_sigma2s_all), k, nexcp
	end if
