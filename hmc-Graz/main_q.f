C     ******************************************************************
      program main_quenched
C     ******************************************************************
C     **                    **                                        **
C     **  QUENCHED          **    I. Hip 13 Mai 97                    **
C     **                    **    Last modified: 13 Mai 97 (ivh)      **
C     **                    **                                        **
C     ******************************************************************
...
C     >>> simulation parameters
      real*8 beta, eps, akap

C     >>> needed to call update
      integer*4 update
      real*8 s_final, h_delta

C     >>> variables used for some measurements
      real*4 eign(3)
      real*8 cumtime, detm
...
C     Initialization
C     ------------------------------------------------------------------
C     >>> quenched means \kappa = 0
      akap = 0.0d0

C     >>> interactive parameter input
      write(*, *)
      write(*, *) 'Quenched - v3.0 (13 Mai 97)'
      write(*, *) '~~~~~~~~~~~~~~~~~~~~~~~~~~~'
      write(*, *)
      write(*, '(1x, ''start beta     = '', $)')
      read(*, *) beta_start
...
5     write(*, '(1x, ''1=nu and S_g, 2=EV-s of Q = '', $)')
      read(*, *) imode
      if((imode .lt. 1) .or. (imode .gt. 2)) goto 5
...
C     >>> prepare textual data description
      ...
      c_computable(7) = 'cummulative time'
      c_computable(8) = 'num. of real ev-s'
      c_computable(9) = 'smallest abs(ev(Q))'
      c_computable(10) = '2nd smallest'
      c_computable(11) = '3rd smallest'
...
C       >>> measurements
	...
	computable(6, ns) = sngl(dexp(-h_delta))
        computable(7, ns) = sngl(cumtime)

	if(imode .eq. 2) then
c	  computable(8, ns) = float(nageig(akap, detm, eign))
c	  computable(9, ns) = eign(1)
c	  computable(10, ns) = eign(2)
c	  computable(11, ns) = eign(3)
	else
	  computable(8, ns) = 0.0
	  computable(9, ns) = 0.0
	  computable(10, ns) = 0.0
	  computable(11, ns) = 0.0
	end if
